...
export function buildDemoExport(): string {
  const data = {
    ...
    tasks: [
      task("d1", "把这个网页优化一下", 1, 3, false, 1, { deep: true, product: true }),
      task("d2", "和 UI 设计师对一下方案", 2, 1, false, 0, { product: true }),
      task("d3", "和技术负责人对一下当前 version", 3, 1, false, 0, { product: true }),
      task("d4", "建一个数据库", 4, 2, false, 0, { deep: true, product: true }),
      task("d5", "学一下 AI Go-To-Market", 5, 2, false, 0),
      task("d6", "去练瑜伽", 6, 1, false, 0, { health: true }),
      task("d7", "去探一家店", 7, 1, false, 0),
      task("d8", "做一个行业 research", 8, 2, true, 2, { revenue: true }),
      task("d9", "找一下这家公司的 PitchDeck", 9, 1, true, 1, { revenue: true }),
      task("d10", "打电话给 AT&T", 10, 1, true, 1),
      task("d11", "和团队碰一下", 11, 1, true, 1),
      task("d12", "遛狗", 12, 1, true, 1, { health: true }),
    ],
    activeTaskId: null,
    planConfirmed: true,
    aiSource: "ai",
    today: dayLog(dayKey(0), 78, 7, 175, ["morning-upgrade"], [
      "做一个行业 research",
      "找一下这家公司的 PitchDeck",
      "打电话给 AT&T",
      "和团队碰一下",
